...
## Evidence: 2026-07-29 Active Delegate and Leave Tracking
- `/LeaveTracking` follows the existing authenticated management shell and exposes one current workflow: month navigation, elevated department filtering, pending/approved summaries, a seven-day desktop calendar, selected-day workforce status, and manual transfer only for the current active delegate.
...
- The isolated MSSQL/Chromium E2E approved a manager leave, observed its green calendar state with request and approval actors, exercised manual active-delegate transfer without another leave, verified report and pending-approval reassignment, restored the primary manager, and included `/LeaveTracking` in desktop, tablet, mobile, short-mobile, and landscape overflow checks.

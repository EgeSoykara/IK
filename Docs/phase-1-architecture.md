...
- `Models/EmployeeProfilePhoto.cs`, `Models/EmployeeDocument.cs`, and `Models/EmployeeDocumentCategory.cs` own employee-file metadata. `EmployeeDocumentCategory.CanonicalKey` is both the stable category identity and the document-folder segment; original file names are display/download metadata only and never storage paths.
- `Services/EmployeeFileService.cs` is the single profile-photo and document operation authority. It resolves the current employee from claims, uses a fresh `HumanResourcesDbContext` per operation, validates ownership, file size, extension and signature, persists file metadata, records upload audit events, replaces the previous profile photo, and exposes only current-employee reads. Profile-photo metadata uses SQL Server `rowversion`, so concurrent replacements cannot leave a losing metadata write as a second authority.
...
- `Program.cs` owns ASP.NET Core cookie authentication registration, the `/auth/login` and `/auth/logout` endpoints, and the authenticated `/employee-files/profile-photo` plus `/employee-files/documents/{documentId}` current-employee read endpoints.
...
- `Components/Pages/Home.razor` is the authenticated employee dashboard at `/`. It may resolve the summary employee from the current user's `EmployeeId` claim or from the `employeeId` query string, but cross-employee query-string access remains a `PageAccessService` permission decision. Leave balances and requests remain read-only summaries. Profile-photo and document controls render only when the selected employee is the current claim employee and delegate every file operation to `EmployeeFileService`; viewing another employee never exposes that employee's files. The total remaining-days dashboard metric uses `Services/LeaveBalanceDashboardSummary.cs` to count only the latest yearly balance for each leave type, because a newer yearly balance may already include the previous year's remaining days as carry-over. The leave-request form uses that same summary authority to preview the employee's projected total remaining days after the selected request is approved.
- `Components/Pages/Home.razor` offers only active categories as upload targets but groups existing documents from their own stored category metadata, so deactivating a category cannot hide historical files. One shared file-operation gate disables both upload controls while either upload is active, so dashboard reloads cannot overlap on the circuit context.
...
14. The dashboard renders file controls only when its selected employee matches the authenticated `EmployeeId` claim.
15. `EmployeeFileService` validates the selected file and category, creates an opaque canonical storage key, atomically writes bytes outside `wwwroot`, then commits metadata and an audit event through a fresh operation context. A failed or cancelled metadata commit performs non-cancelled file compensation.
16. Replacing a profile photo commits the new metadata before removing the superseded file; documents remain grouped by their DB-backed category canonical key.
17. Authenticated download endpoints resolve only current-employee metadata before opening the canonical storage key; user-supplied names never participate in path resolution.

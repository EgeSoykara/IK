...
Talep formundaki `Geriye dönük izin talebi` seçimi kapalıyken geçmiş tarih seçilemez. Seçim açıkça işaretlendiğinde çalışan geçmişteki bir çalışma günü veya aralığı için talep oluşturabilir; bu kayıt `LeaveRequest.IsRetrospective` ile kalıcı tutulur ve normal müdür → İK onay, bakiye yeterliliği, çakışma ve iş günü hesaplama kurallarının tamamından geçer.

İzin iptalinde iki ayrı iş kuralı vardır. Talep hâlâ müdür kararı bekliyorsa çalışan kendi talebini doğrudan iptal eder; bekleyen onay satırı kaldırılır, talep `Cancelled` olur, bakiye değişmez ve bu işlem karar geçmişi için doğrudan iptal kaydı üretir. Müdür talebi onaylayıp İK'ya gönderdikten sonra doğrudan iptal kapalıdır: çalışan asıl sürecin sonuçlanmasını bekler. Yalnız İK tarafından onaylanmış izin için iptal başlangıç/bitiş tarihleri ve gerekçeyle iptal talebi oluşturulabilir. Başlangıç ve bitiş aynı gün olabilir; bu yalnız o izin gününün iptalidir. İptal talebi önce talebin atanmış departman müdürüne, ardından İK'ya gider; ret gerekçesi zorunludur.

İK nihai onayı sırasında bakiyeden düşülen her tarih ve miktar `LeaveRequestApprovedDays` içinde saklanır. İptal iadesinin tek gün otoritesi bu onay anı provenansıdır; sonradan resmî tatil eklenmesi veya kaldırılması geçmişte onaylanmış günleri yeniden hesaplamaz. `CancellationStartDate`–`CancellationEndDate` aralığındaki kalan provenans satırları toplanır; onaylanan iptalde bu satırlar kaldırılır. Önceden onaylanmış veya hâlen onay bekleyen bir iptal aralığıyla çakışan yeni iptal talebi reddedilir. İade edilen günler, asıl onayın değişmez `LeaveRequestBalanceAllocations` kaynak provenansının ters sırasıyla bakiyedeki `UsedDays` değerinden düşülür ve `LeaveCancellationBalanceRefunds` içinde saklanır. Bütün kalan günler iade edilmişse asıl talep `Cancelled` olur. Kısmi veya orta aralık iptalinde asıl başlangıç/bitiş korunur, yalnız `RequestedDays` ve kalan onaylı gün provenansı azaltılır; izin takip, iş gücü durumu ve yeni talep çakışması aynı kalan gün kayıtlarını okur. Yönetici vekâleti ise seçilen iptal aralığında durur ve aralığın dışındaki izin günlerinde devam eder. Böylece 17–23 Ağustos iptali 15–16 ve 24–25 Ağustos izinlerini korur; 20–20 Ağustos yalnız 20 Ağustos'u iptal eder.

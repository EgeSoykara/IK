...
İzin iptalinde iki ayrı iş kuralı vardır. Talep hâlâ müdür kararı bekliyorsa çalışan kendi talebini doğrudan iptal eder; bekleyen onay satırı kaldırılır, talep `Cancelled` olur ve henüz düşüm yapılmadığı için bakiye değişmez. Müdür talebi onaylayıp İK'ya gönderdikten sonra doğrudan iptal kapalıdır: çalışan asıl sürecin sonuçlanmasını bekler. Yalnız İK tarafından onaylanmış izin için işe dönüş tarihi ve gerekçeyle iptal talebi oluşturulabilir. İptal talebi önce talebin atanmış departman müdürüne, ardından İK'ya gider; ret gerekçesi zorunludur.

İK iptal onayında kullanılan günleri izin başlangıcından `ReturnDate` öncesine kadar güncel resmî tatil takvimiyle yeniden hesaplar. İade, onaylı talebin mevcut `RequestedDays` değerinden bu kullanılan günlerin çıkarılmasıyla bulunur; böylece sonradan değişen tatil takvimi nedeniyle asıl onayda tahsis edilenden fazla gün iade edilemez. Yalnız bu günler, asıl onayın değişmez `LeaveRequestBalanceAllocations` provenansının ters sırasıyla bakiyedeki `UsedDays` değerinden düşülür; her iade `LeaveCancellationBalanceRefunds` içinde bakiye ve kaynak bazında ayrıca saklanır. İznin tamamı iade edilmişse asıl talep `Cancelled` olur. Kısmi erken dönüşte asıl talep onaylı kalır, bitiş tarihi işe dönüş gününden bir önceki güne çekilir ve talep günü yalnız kullanılan günlere indirilir. Böylece kullanılmış günler bakiyeden düşülmüş kalır; yalnız kullanılmayan günler iade edilir.

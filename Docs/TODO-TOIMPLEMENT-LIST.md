...
- [x] Let authenticated employees upload their own profile photo and categorized documents from the dashboard without granting cross-employee file access. Evidence: `Components/Pages/Home.razor`, `Services/EmployeeFileService.cs`, and `Tests/EmployeeFileServiceTests.cs`.
...
- [x] Keep employee-file lifecycle consistent through atomic writes, per-operation DbContexts, profile-photo concurrency control, a shared dashboard upload gate, inactive-category history visibility, and database-enforced employee-deletion blocking while personal files exist. Evidence: `Services/LocalEmployeeFileStore.cs`, `Services/EmployeeFileService.cs`, `Models/EmployeeProfilePhoto.cs`, `Database/HumanResourcesDbContext.cs`, `Components/Pages/Home.razor`, `Components/Pages/Employees.razor`, and focused tests.
